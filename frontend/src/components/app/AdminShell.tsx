import {
  ...
  Landmark, Layers3, LogOut, Menu, Moon, ReceiptText, RefreshCcw, Search, Settings, ShieldCheck, Sun, UserCog, Users,
...
const navGroups: NavGroup[] = [
  ...
  { label: "Administração", items: [
    { path: "/configuracoes/usuarios", label: "Usuários", icon: UserCog, adminOnly: true },
    { path: "/sincronizacao", label: "Sincronização", icon: RefreshCcw, adminOnly: true },
    { path: "/configuracoes", label: "Configurações", icon: Settings, adminOnly: true },
  ] },
];

const allItems = navGroups.flatMap((group) => group.items);
export const INTEGRATION_LABEL = "Integração real · API + Supabase";
...
export function AdminShell({ path, navigate, user, onLogout, children }: { path: string; navigate: (path: string) => void; user: AppUser; onLogout: () => Promise<void>; children: ReactNode }) {
  ...
  const visibleGroups = navGroups.map((group) => ({ ...group, items: group.items.filter((item) => !item.adminOnly || user.role === "ADMIN") })).filter((group) => group.items.length > 0);
  const sidebar = <div className="flex h-full flex-col bg-[#071525] text-slate-100"><div className="flex h-20 items-center border-b border-white/10 px-5"><div className="flex size-10 items-center justify-center rounded-xl bg-emerald-400 font-black text-[#071525]">RF</div><div className="ml-3"><p className="font-semibold">Remo Funding</p><p className="text-[10px] uppercase tracking-widest text-slate-400">Gestão de capital</p></div></div><div className="px-4 pt-4"><Badge variant="warning" className="w-full justify-center py-1.5">Ambiente de desenvolvimento</Badge></div><nav className="mt-4 flex-1 space-y-1 overflow-y-auto px-3">{visibleGroups.map((group, index) => group.label ? <div key={group.label}><button type="button" onClick={() => setGroups((current) => ({ ...current, [group.label!]: current[group.label!] === false }))} className="flex h-9 w-full items-center px-3 text-xs font-semibold uppercase tracking-widest text-slate-500">{group.label}<ChevronDown className={cn("ml-auto size-3", groups[group.label] === false && "-rotate-90")} /></button>{groups[group.label] !== false && <div className="ml-2 space-y-1 border-l border-white/10 pl-2">{group.items.map((item) => <NavEntry key={item.path} item={item} path={path} navigate={navigate} />)}</div>}</div> : group.items.map((item) => <NavEntry key={`${index}-${item.path}`} item={item} path={path} navigate={navigate} />))}</nav><div className="border-t border-white/10 p-4 text-xs text-slate-400"><p className="font-medium text-slate-200">{user.name}</p><p>{user.role === "ADMIN" ? "Administrador" : "Analista"}</p></div></div>;
  return <div className="min-h-screen bg-background"><aside className="fixed inset-y-0 left-0 z-40 hidden w-[280px] lg:block">{sidebar}</aside>{mobile && <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 bg-black/70" onClick={() => setMobile(false)} /><aside className="relative h-full w-[300px]">{sidebar}<Button size="icon" variant="ghost" className="absolute right-2 top-2" onClick={() => setMobile(false)}><X className="size-5" /></Button></aside></div>}<div className="lg:pl-[280px]"><header className="sticky top-0 z-30 flex h-20 items-center gap-3 border-b bg-background/90 px-4 backdrop-blur sm:px-6"><Button size="icon" variant="ghost" className="lg:hidden" onClick={() => setMobile(true)}><Menu className="size-5" /></Button><div className="flex-1"><Breadcrumbs path={path} navigate={navigate} /></div><label className="relative hidden max-w-sm flex-1 md:block"><Search className="absolute left-3 top-3 size-4 text-muted-foreground" /><Input className="pl-9" placeholder="Buscar no ambiente de desenvolvimento…" /></label><div className="hidden text-right text-xs sm:block"><p className="font-medium">{user.name}</p><p className="text-muted-foreground">{user.role}</p></div><Button size="icon" variant="ghost"><Bell className="size-5" /></Button><Button size="icon" variant="outline" onClick={() => setTheme((value) => value === "dark" ? "light" : "dark")}>{theme === "dark" ? <Sun className="size-4" /> : <Moon className="size-4" />}</Button><Button variant="outline" onClick={() => void onLogout()}><LogOut className="size-4" /><span className="hidden sm:inline">Sair</span></Button></header><main className="mx-auto max-w-[1680px] p-4 sm:p-6 lg:p-8">{children}</main><footer className="border-t px-6 py-5 text-xs text-muted-foreground">Remo Funding · Integração operacional via API e Supabase</footer></div></div>;
}
