...
interface FormState { investor_id: string; contribution_date: string; end_date: string; original_amount: string; monthly_rate: string; status: ContributionStatus; notes: string; }

function initial(item: FundingContribution | undefined, investors: FundingInvestor[], presetInvestorId?: string): FormState {
  if (item) return { investor_id: item.investor_id, contribution_date: item.contribution_date, end_date: item.end_date ?? "", original_amount: decimalToBrazilianInput(item.original_amount), monthly_rate: item.monthly_rate.replace(".", ","), status: item.status, notes: item.notes ?? "" };
  return { investor_id: presetInvestorId ?? investors[0]?.id ?? "", contribution_date: new Date().toISOString().slice(0, 10), end_date: "", original_amount: "", monthly_rate: "0,02", status: "ACTIVE", notes: "" };
}
...
export function ContributionFormModal({ open, contribution, investors, presetInvestorId, saving = false, onClose, onSave }: { open: boolean; contribution?: FundingContribution; investors: FundingInvestor[]; presetInvestorId?: string; saving?: boolean; onClose: () => void; onSave: (input: FundingContributionInput) => void }) {
  ...
  const valid = Boolean(form.investor_id && form.contribution_date && (!form.end_date || form.end_date >= form.contribution_date) && amount && BigInt(amount.replace(".", "")) > 0n && validRate);
  const submit = () => { if (valid && amount) onSave({ investor_id: form.investor_id, contribution_date: form.contribution_date, end_date: form.end_date || null, original_amount: amount, monthly_rate: rate, status: form.status, notes: form.notes || null }); };
  return <Modal open={open} title={contribution ? `Editar ${contribution.code}` : "Novo aporte"} description="Taxa armazenada como fração decimal: 2% a.m. = 0,02. O valor original fica auditado." onClose={onClose} footer={<><Button variant="outline" onClick={onClose}>Cancelar</Button><Button disabled={!valid || saving} onClick={submit}>{saving ? "Salvando…" : "Salvar aporte"}</Button></>}>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <FormField label="Data fim" hint="Vencimento contratual; não registra devolução automática."><Input type="date" min={form.contribution_date} value={form.end_date} onChange={(event) => setForm((current) => ({ ...current, end_date: event.target.value }))} /></FormField>
