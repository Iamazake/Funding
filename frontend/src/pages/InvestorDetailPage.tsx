...
export function InvestorDetailPage({ id, navigate }: { id: string; navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <Card className="grid gap-4 bg-card/75 p-5 sm:grid-cols-3"><Info label="CPF / CNPJ" value={investor.tax_id_masked ?? "Não informado"} /><Info label="Telefone" value={investor.phone ?? "Não informado"} /><Info label="Observação" value={investor.notes ?? "Sem observações"} /></Card>
    ...
    {contributions.length === 0 ? <EmptyState title="Nenhum aporte cadastrado." description="Este investidor ainda não possui aportes." /> : <Card className="overflow-hidden bg-card/75"><Table><TableHeader><TableRow><TableHead>Aporte</TableHead><TableHead>Data do aporte</TableHead><TableHead>Vencimento</TableHead><TableHead>Valor original</TableHead><TableHead>Taxa mensal</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{contributions.map((item) => <TableRow key={item.id} className="cursor-pointer" onClick={() => navigate(`/cadastro/aportes/${item.id}`)}><TableCell className="font-mono text-xs">{item.code}</TableCell><TableCell>{formatDate(item.contribution_date)}</TableCell><TableCell>{item.end_date ? formatDate(item.end_date) : "Não informado"}</TableCell><TableCell>{formatMoney(item.original_amount)}</TableCell><TableCell>{formatMonthlyRate(item.monthly_rate)}</TableCell><TableCell><StatusBadge status={item.status} /></TableCell></TableRow>)}</TableBody></Table></Card>}
...
function Info({ label, value }: { label: string; value: string }) { return <div><p className="text-xs uppercase tracking-wider text-muted-foreground">{label}</p><p className="mt-1 text-sm font-medium">{value}</p></div>; }
