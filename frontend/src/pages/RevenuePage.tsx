...
export function RevenuePage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Receita · dados operacionais" title="Parcelas e recebimentos" description="Registros reais de ECON_AMORTIZACOES. Linhas repetidas da mesma parcela permanecem visíveis e independentes." />
    ...
    {view === "success" && state.status === "success" && <><div className="hidden md:block"><Card className="overflow-hidden bg-card/75"><Table className="min-w-[1450px]"><TableHeader><TableRow><TableHead>Contrato / cliente</TableHead><TableHead>Parcela</TableHead><TableHead>Vencimento</TableHead><TableHead>Pagamento</TableHead><TableHead>PMT</TableHead><TableHead>Pago</TableHead><TableHead>Principal</TableHead><TableHead>Juros</TableHead><TableHead>Desconto</TableHead><TableHead>Situação</TableHead><TableHead>Qualidade</TableHead><TableHead>Funding</TableHead></TableRow></TableHeader><TableBody>{state.data.items.map((row) => <TableRow key={row.id} className="cursor-pointer" onClick={() => navigate(`/receita/${row.id}`)}><TableCell><AppLink to={`/receita/${row.id}`} onNavigate={navigate} className="font-semibold text-primary">{row.contract_code ?? "Contrato não informado"}</AppLink><p className="text-xs text-muted-foreground">{row.client_name ?? "Cliente não resolvido"}</p></TableCell><TableCell>{row.installment_code ?? "—"}</TableCell><TableCell>{formatDate(row.due_date)}</TableCell><TableCell>{formatDate(row.payment_date)}</TableCell><TableCell>{formatOperationalMoney(row.expected_amount)}</TableCell><TableCell>{formatOperationalMoney(row.paid_amount)}</TableCell><TableCell>{formatOperationalMoney(row.principal_component)}</TableCell><TableCell>{formatOperationalMoney(row.interest_component)}</TableCell><TableCell>{formatOperationalMoney(row.discount_amount)}</TableCell><TableCell><StatusBadge status={row.situation ?? row.installment_status ?? "NÃO INFORMADA"} /></TableCell><TableCell><QualityIndicator row={row} /></TableCell><TableCell>Não informado</TableCell></TableRow>)}</TableBody></Table></Card></div><div className="grid gap-4 md:hidden">{state.data.items.map((row) => <Card key={row.id} className="bg-card/75"><CardContent className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><AppLink to={`/receita/${row.id}`} onNavigate={navigate} className="font-semibold text-primary">{row.contract_code ?? "Contrato não informado"}</AppLink><p className="text-xs text-muted-foreground">Parcela {row.installment_code ?? "—"} · {row.client_name ?? "Cliente não resolvido"}</p></div><StatusBadge status={row.data_quality_status} /></div><div className="grid grid-cols-3 gap-2 text-sm"><Metric label="Pago" value={formatOperationalMoney(row.paid_amount)} /><Metric label="Principal" value={formatOperationalMoney(row.principal_component)} /><Metric label="Juros" value={formatOperationalMoney(row.interest_component)} /></div></CardContent></Card>)}</div><Pagination page={state.data.pagination.page} pages={state.data.pagination.pages} total={state.data.pagination.total} onPage={(page) => update({ page })} /></>}
  </div>;
}
