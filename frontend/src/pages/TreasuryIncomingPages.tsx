...
export function TreasuryIncomingListPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Tesouraria" title="Entradas esperadas" description="Parcelas baixadas, conferência manual no banco, conciliação e rateio do recebimento." />
...
export function TreasuryIncomingDetailPage({ id, navigate }: { id: string; navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    <AppLink to="/tesouraria/entradas" onNavigate={navigate} className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="size-4" />Voltar</AppLink>
