...
const bankValidationLabels = {
  NOT_RECORDED: "Pendente / não registrada",
  VALIDATED: "Validada",
...
export function RevenueDetailPage({ id, navigate }: { id: string; navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6"><AppLink to="/receita" onNavigate={navigate} className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="size-4" />Voltar para Receita</AppLink><PageHeader eyebrow="Receita operacional" title={`${row.contract_code ?? "Contrato não informado"} · parcela ${row.installment_code ?? "—"}`} description={row.client_name ?? "Cliente não resolvido na promoção atual."} actions={<><StatusBadge status={row.data_quality_status} /><StatusBadge status={distribution.status} /></>} /><FeedbackBanner feedback={feedback} onClose={() => setFeedback(null)} /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><KpiCard compact icon={ReceiptText} label="Valor esperado" value={formatOperationalMoney(row.expected_amount)} /><KpiCard compact icon={Banknote} label="Valor pago" value={formatOperationalMoney(row.paid_amount)} /><KpiCard compact icon={Landmark} label="Principal" value={formatOperationalMoney(row.principal_component)} /><KpiCard compact icon={AlertTriangle} label="Avisos / divergências" value={`${row.warning_count} / ${row.divergence_count}`} /></div><RevenueFundingPanel distribution={distribution} saving={saving} onDistribute={distribute} onReverse={reverse} /><InfoGrid rows={[["Contrato", row.contract_code ?? "Não informado"], ["Cliente", row.client_name ?? "Não resolvido"], ["Parcela", row.installment_code ?? "Não informada"], ["Vencimento", formatDate(row.due_date)], ["Data da baixa", formatDate(row.payment_date)], ["Juros", formatOperationalMoney(row.interest_component)], ["Desconto", formatOperationalMoney(row.discount_amount)], ["Status da parcela", row.installment_status ?? "Não informado"], ["Situação", row.situation ?? "Não informada"], ["Antecipação", row.anticipation_marker ?? "Não informada"], ["Marcador BAIXA_TOTAL", row.payment_marker ?? "Vazio"], ["Referência operacional", row.source_reference ?? "Não informada"], ["Funding", row.funding_status ?? "Não vinculado"], ["Validação bancária", bankValidationLabels[row.bank_validation_status]]]} /><QualityPanel warnings={row.warnings} divergences={row.divergences} /></div>;
}
