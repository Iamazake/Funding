...
export function ContributionsPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    {state.status === "success" && <>
      ...
      {contributions.length === 0 ? <EmptyState title="Nenhum aporte cadastrado." description="Registre o primeiro aporte pela API real." /> : filtered.length === 0 ? <EmptyState title="Nenhum aporte encontrado." description="Ajuste os filtros da consulta." /> : <Card className="overflow-hidden bg-card/75"><Table><TableHeader><TableRow><TableHead>Aporte</TableHead><TableHead>Investidor</TableHead><TableHead>Data do aporte</TableHead><TableHead>Data fim</TableHead><TableHead>Valor original</TableHead><TableHead>Taxa</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{filtered.map((item) => <TableRow key={item.id}><TableCell className="font-mono text-xs">{item.code}</TableCell><TableCell>{investors.find((value) => value.id === item.investor_id)?.name ?? "—"}</TableCell><TableCell>{formatDate(item.contribution_date)}</TableCell><TableCell>{item.end_date ? formatDate(item.end_date) : "Não informado"}</TableCell><TableCell>{formatMoney(item.original_amount)}</TableCell><TableCell>{formatMonthlyRate(item.monthly_rate)}</TableCell><TableCell><StatusBadge status={item.status} /></TableCell><TableCell><div className="flex justify-end gap-2"><Button size="sm" variant="ghost" onClick={() => { setEditing(item); setOpen(true); }}><Pencil className="size-4" /><span className="sr-only">Editar</span></Button><AppLink to={`/cadastro/aportes/${item.id}`} onNavigate={navigate}><Button size="sm" variant="outline"><Eye className="size-4" />Ver</Button></AppLink></div></TableCell></TableRow>)}</TableBody></Table></Card>}</>}
    </>}
